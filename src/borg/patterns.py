...
class ShellPattern(PatternBase):
    ...
    def _match(self, path, wrapped_path):
        return self.regex.match(wrapped_path) is not None
        # matched = self.regex.match(wrapped_path) is not None
        # print(f"sh: Matched re='{self.regex.pattern}' against '{wrapped_path}' with re.match, got {matched}")
        # return matched
