...
def load_suite(files)
  ...
  files.each do |path|
    ...
    total_time << last_line[TOTAL_RE, 1].to_i
  end

  [stats, total_time]
end

a, a_time = load_suite(a_files)
b, b_time = load_suite(b_files)
...
a.keys.each do |k|
  ...
  puts [
    ...
    "#{a_avg.round(2)} (+-#{a_stddev.round(1)})",
    "#{b_avg.round(2)} (+-#{b_stddev.round(1)})",
    (b_avg - a_avg).round(2),
...
puts "Total time a=#{a_time_avg.round(2)} (+-#{a_time_stddev.round(1)}) b=#{b_time_avg.round(2)} (+-#{b_time_stddev.round(1)}) diff=#{(b_time_avg - a_time_avg).round(2)} (#{-(100 - (b_time_avg / a_time_avg * 100)).round}%)"
